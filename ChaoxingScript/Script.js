...
function check_page() {
    try {
        ...
        if (blk.title == "章节测验" || blk.title == "作业" || blk.title == "练习题") {
...
function change_to_answer() {
    try {
        ...
        //如果是章节检测
        if (blk.title == "章节测验" || blk.title == "作业") {
            blk.click();
            config.changing = false;
            player = undefined;
            //开始答题
...
function removeCode() {
    ...
    //移除登录界面二维码
    try {
        var code = document.getElementById("notice_box_0x003");
        document.getElementById("body_0x003").removeChild(code);
    } catch (e) { }

    //课程页面左下角二维码
    try {
        var code = document.getElementsByClassName("ewm")[0];
        document.getElementsByClassName("funclist")[0].removeChild(code);
    } catch (e) { }
